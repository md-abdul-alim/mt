...
function RejectedMachines() {
  ...
  async function fetchMachines() {
    ...
    try {
      await axios
        .get(`/api/rejected/machine/per/unit/list/${localStorage.getItem("unit_id")}/`, AxiosHeader)
        .then((res) => {
  ...
  const postMachine = async (values, setSubmitting) => {
    try {
      await axios
        .post("/api/rejected/machine/create/", values, AxiosHeader)
        .then((resp) => {
        ...
        customBodyRender: (value, tableMeta, updateValue) => {
          ...
          return (
            <>
              ...
              <Link
                to={{
                  pathname: "/app/rejected/machine/details",
                  state: item.id,
                }}
              >
    ...
    <React.Fragment>
      <Box position="relative">
        <MuiThemeProvider theme={getMuiTheme()}>
          <div>
            <BreadCrumb routeSegments={[{ name: "Rejected Machines" }]} />

            <Paper className={classes.pageContent}>
              <MachineFilterForm
                addOrEdit={filterFunction}
              />
            </Paper>
...
export default RejectedMachines;
