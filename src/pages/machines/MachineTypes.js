...
export default function MachineTypes() {
  ...
  async function fetchMachineTypes() {
    ...
    try {
      await axios
        .get("/api/machine/type/list/", AxiosHeader)
        .then((res) => {
          setMachineTypeList(res.data);
        })
        .catch((err) => {
          console.log(err);
        });
  ...
  const postFabricType = async (values, setSubmitting) => {
    ...
    try {
      await axios
        .post("/api/machine/type/create/", values, AxiosHeader)
        .then((resp) => {
  ...
  const updateFabricType = async (values, setSubmitting) => {
    ...
    try {
      await axios
        .put(`/api/machine/type/update/${values.id}/`, values, AxiosHeader)
        .then((resp) => {
          setFabricTypeRecord(resp.data);
          setSubmitting(false);
        });
  ...
  return (
    ...
    <div>
      ...
      />
      <Popup
        title="Machine Type Form"
        openPopup={openPopup}
        setOpenPopup={setOpenPopup}
      >
        <MachineTypeForm recordForEdit={recordForEdit} addOrEdit={addOrEdit} machineTypeList={machineTypeList} />
      </Popup>
