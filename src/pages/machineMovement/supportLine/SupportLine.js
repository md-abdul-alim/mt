...
export default function SupportLine() {
  ...
  async function fetchCompositions() {
    try {
      setIsLoading(true);
      await axios
        .get(`/api/support/line/machine/list/${unit}/`, AxiosHeader)
        .then((res) => {
  ...
  const addLine = async (values, setSubmitting) => {
    try {
      await axios
        .post(`/api/add/machine/to/support/line/`, values, AxiosHeader)
        .then((resp) => {
  ...
  const updateLine = async (values, setSubmitting) => {
    try {
      await axios
        .put(`/api/support/machine/line/update/`, values, AxiosHeader)
        .then((resp) => {
          setLineRecord(resp.data);
          setSubmitting(false);
        });
  ...
  const removeMachineFromLine = async (values) => {
    await axios
      .delete(`/api/remove/machine/from/support/line/`, {
        params: { id: values.id },
        headers: {
          "Authorization": `Bearer ${localStorage.getItem("access_token")}`
        },
              ...
              <IconButton
                ...
                onClick={() => {
                  if (item.unit_name === unit && (user_type === 'Maintenance Head' || user_type === 'Maintenance Coordinate' || user_type === 'Admin')){
                    ...
                    alert("You have no ideal permission.")
                  }
                }}
              >
                <Controls.Button
